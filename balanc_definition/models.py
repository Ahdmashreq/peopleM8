...
class Cost_Center(models.Model):
    ...
    cost_center = models.PositiveSmallIntegerField(blank=True, null=True, verbose_name=_('Cost Center'))
    account_number = models.PositiveSmallIntegerField(blank=True, null=True, verbose_name=_('Account Number'))
    created_by =            models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,related_name="cost_center_created_by")
    creation_date	=	    models.DateField(auto_now=True, auto_now_add=False)
    last_update_by	=	    models.ForeignKey(settings.AUTH_USER_MODEL, blank=True, null=True, on_delete=models.CASCADE,related_name="cost_center_last_update_by")
    last_update_date	=	models.DateField(auto_now=False, auto_now_add=True)
...
class Cost_Center_Link(models.Model):
    cost_center = models.ForeignKey(Cost_Center, on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('Element Name'))
    payroll = models.ForeignKey(Payroll_Master, on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('Payroll'))
    department = models.ForeignKey(Department, on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('Department'))
    job = models.ForeignKey(Job, on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('Job'))
    grade = models.ForeignKey(Grade, on_delete=models.CASCADE, null=True, blank=True, verbose_name=_('Grade'))
