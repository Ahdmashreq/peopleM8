...
def message_composer(request, html_template, instance_name, result):
    reviewed_by = Employee.objects.get(user=request.user)   #manager
    employee = Employee.objects.get(user=instance_name.user)  # leave employee regadless login user
